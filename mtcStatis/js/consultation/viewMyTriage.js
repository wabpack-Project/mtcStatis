...
var flag = searchArr[searchArr.indexOf('flag') + 1]
$(function () {
	$.ajax({
		url: baseurl + 'consultationTriage/getConsultationMessage_lookNew?consultationNo=' + consultationNo + '&flag=' + flag,
		type: 'get',
		success: function (msg) {
			...
			$('.triage-box .consul-time span').html(consultationInfo.startTime ? new Date(consultationInfo.startTime).Format('yyyy-MM-dd hh:mm') : '')
			var doctorList = msg.doctorList
			if (doctorList && doctorList.length) {
				var doctorHtml = ''
				for (var i = 0; i < doctorList.length; i++) {
					doctorHtml += '<tr><td>' + doctorList[i].doctorName + '</td><td>' + doctorList[i].doctorTitle + '</td><td>' + doctorList[i].hospitalName + '</td><td>' + doctorList[i].departmentName + '</td><td>' + doctorList[i].triMain + '</td><td>'
					doctorHtml += doctorList[i].message ? doctorList[i].message : ''
					doctorHtml += '</td></tr>'
				}
				$('.triage-box .desMtc-box tbody').html(doctorHtml)
			}
			// 流程状态
	...
	}).on('click', '.button-box .send-message', function () {
		// 发送通知短信
		var ghConsultationInfo = {}
		ghConsultationInfo.state = 20 //20待分诊->30待会诊流程
		ghConsultationInfo.consultationNo = consultationNo
		$.ajax({
			url: baseurl + "/consultation/sendMsg",
			dataType: "json",
			type: 'POST',
			cache: false,
			contentType: "application/json",
			data: JSON.stringify(ghConsultationInfo),
