...
function getDesMtc () {
	$.ajax({
		url: baseurl + 'consultationTriage/queryMtcResponsibility',
		type: 'post',
		data: {},
		dataType: 'json',
		success: function (desMtcData) {
			var data = desMtcData.result
			var desMtcHtml = '<option value="">-请选择-</option>'
			for (var i = 0; i < data.length; i++) {
				desMtcHtml += '<option value="' + data[i].id + '">' + data[i].mtc_name + '</option>'
			}
			$('.triage-box .desMtc-box select').html(desMtcHtml)
		},
		error: function (err) {
			console.log(err)
		}
...
$(function () {
	getDesMtc()
	$.ajax({
		url: baseurl + 'consultationTriage/getConTransferMessageNew?consultationNo=' + consultationNo,
		type: 'get',
