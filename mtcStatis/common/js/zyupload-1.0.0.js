...
//					edit             : true,  						      // 是否可以编辑文件（裁剪）
			...
			this.createHtml = function(){
				...
				var html= '<form id="uploadForm" action="'+para.url+'" method="post" enctype="multipart/form-data"><div class="upload-wrap border_box clearfix padding_20"><div class="select-box border_box padding_20"'
				if (!para.dragDrop) {
					html += ' style="width:100%;"'
				}
				html += '><div class="image"></div><p><a class="button button-green">点击选择文件</a></p><input id="fileImage" type="file" ' + multiple + '></div>'
				if(para.dragDrop){
					// 创建带有拖动的html
					html += '<div class="drag-select border_box padding_20" id="fileDragArea"><p>或者将文件拖至此处</p></div>'
					html += '</form>';
				}
				html += '</div><div class="button-box"><a class="button button-primary select-continue">继续选择</a><a class="button button-green besure-upload" id="fileSubmit">开始上传</a></div><div id="preview" class="clearfix padding_20 border_box"></div></form>';
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				type = type.toLowerCase();
				// 处理不同类型文件代表的图标
				var fileImgSrc = "/mtcStatis/images/";
				if(type == "rar"){
					...
					fileImgSrc = fileImgSrc + "7z.png";
				}else if(type == "tif"){
					fileImgSrc = fileImgSrc + "tif.png";
				}else{
				...
				if (file.type.indexOf("image") == 0 && type!="tif") {
				...
				html += '	<input id="fileDescription_'+file.index+'" "type="text" placeholder="添加描述" class="des">';
				html += '</div>';
				return html;
			};
